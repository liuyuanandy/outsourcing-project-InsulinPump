...
class MyPreferenceFragment : PreferenceFragmentCompat(), OnSharedPreferenceChangeListener {
    ...
    @Inject lateinit var zhiKaiPlugin: info.nightscout.pump.danars.ZhiKaiPlugin
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (activity?.let { ContextCompat.checkSelfPermission(it, Manifest.permission.FOREGROUND_SERVICE) } != PackageManager.PERMISSION_GRANTED) {
                activity?.let { ActivityCompat.requestPermissions(it, arrayOf(Manifest.permission.FOREGROUND_SERVICE), 999) }
            }
        }
    }
    ...
    override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
        ...
        if (pluginId != -1) {
            ...
            addPreferencesFromResourceIfEnabled(zhiKaiPlugin, rootKey, config.PUMPDRIVERS)
