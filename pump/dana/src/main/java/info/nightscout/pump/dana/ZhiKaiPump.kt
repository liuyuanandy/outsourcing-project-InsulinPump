...
@Singleton
class ZhiKaiPump @Inject constructor(
    ...
    var blous = 0.0;
    var currentBasal = 0.0//当前基础率

    /*
     * TEMP BASALS
     */
    var isCurrenBasalStop = false;
//    var currentBasal: Double = 0.0
    var tempBasal: Double = 0.0 //临时基础率

    var tempBaselMode:Int = 0;//基础率模式
    ...
    var bolusStartErrorCodeDes: String="" // last start bolus errorCode
    ...
    fun buildZhiKaiProfileRecord(nsProfile: Profile): Array<Double> {
        val record = Array(48) { 0.0 }
        for (hour in 0..23) {
    ...
    fun reset() {
        aapsLogger.debug(LTag.PUMP, "zhikaiPump reset")
        lastConnection = 0
        lastSettingsRead = 0
        readHistoryFrom = 0
    }

    fun modelFriendlyName(): String ="糖脉二代"
