...
import info.nightscout.pump.dana.ZhiKaiFragment
...
@Singleton
class ZhiKaiPlugin @Inject constructor(
    ...
    private val danaPump: ZhiKaiPump,
    ...
    PluginDescription()
        ...
        .fragmentClass(ZhiKaiFragment::class.java.name)
        .pluginIcon(app.aaps.core.ui.R.drawable.icon_logo)
//        .pluginIcon2(app.aaps.core.ui.R.drawable.ic_danars_128)
        .pluginName(info.nightscout.pump.dana.R.string.zhikaipump)
        .shortName(info.nightscout.pump.dana.R.string.zhikaipump_shortname)
        .preferencesId(R.xml.pref_zhikai)
        .description(info.nightscout.pump.dana.R.string.description_pump_zhikaipump),
    injector, aapsLogger, rh, commandQueue
), Pump, Dana, PluginConstraints, OwnDatabasePlugin {

    private val disposable = CompositeDisposable()
    private var danaRSService: ZhiKaiService? = null
    ...
    override fun updatePreferenceSummary(pref: Preference) {
        ...
        if (pref.key == rh.gs(info.nightscout.pump.dana.R.string.key_zhikai_name)) {
            val value = sp.getStringOrNull(info.nightscout.pump.dana.R.string.key_zhikai_name, null)
            pref.summary = value
    ...
    override fun onStart() {
        super.onStart()
        val intent = Intent(context,ZhiKaiService::class.java)
        ...
        disposable += rxBus
            .toObservable(EventZhiKaiDeviceChange::class.java)
            .observeOn(aapsSchedulers.io)
            .subscribe({
    ...
    private val mConnection: ServiceConnection = object : ServiceConnection {
        ...
        override fun onServiceConnected(name: ComponentName, service: IBinder) {
            ...
            val mLocalBinder = service as ZhiKaiService.LocalBinder
    ...
    fun changePump() {
        Log.e("--------------->","changePump")
        mDeviceAddress = sp.getString(info.nightscout.pump.dana.R.string.key_zhikai_address, "")
        mDeviceName = sp.getString(info.nightscout.pump.dana.R.string.key_zhikai_name, "")
        danaPump.serialNumber = sp.getString(info.nightscout.pump.dana.R.string.key_zhikai_name, "")
        danaPump.reset()
    ...
    override fun isInitialized(): Boolean =
        danaPump.lastConnection > 0 && danaPump.maxBasal > 0

    override fun isSuspended(): Boolean =
        danaPump.pumpSuspended || danaPump.errorState != ZhiKaiPump.ErrorState.NONE
    ...
    override fun deliverTreatment(detailedBolusInfo: DetailedBolusInfo): PumpEnactResult {
        ...
        if (detailedBolusInfo.insulin > 0 || carbs > 0) connectionOK = danaRSService?.bolus(detailedBolusInfo, carbs.toInt(), carbTimeStamp,t)
            ?: false
        val result = PumpEnactResult(injector)
    ...
    override fun setTempBasalAbsolute(absoluteRate: Double, durationInMinutes: Int, profile: Profile, enforceNew: Boolean, tbrType: PumpSync.TemporaryBasalType): PumpEnactResult {
        ...
        if (doLowTemp || doHighTemp) {
            ...
            Log.e("-------------------->","percentRate= $percentRate durationInMinutes = $durationInMinutes")
            val result = if (percentRate == 0 && durationInMinutes > 30) {
                Log.e("-------------------->","setTempBasalAbsolute setTempBasalPercent")
                setTempBasalPercent(percentRate, durationInMinutes, profile, enforceNew, tbrType)
            } else {
                Log.e("-------------------->","setTempBasalAbsolute setHighTempBasalPercent")
    ...
    override fun setTempBasalPercent(percent: Int, durationInMinutes: Int, profile: Profile, enforceNew: Boolean, tbrType: PumpSync.TemporaryBasalType): PumpEnactResult {
        ...
        Log.e("----------------->","pumpDescription.maxTempPercent=$pumpDescription.maxTempPercent")
    ...
    override fun cancelTempBasal(enforceNew: Boolean): PumpEnactResult {
        Log.e("-------------------->","danaPump.isTempBasalInProgress = "+danaPump.isTempBasalInProgress)
        if (danaPump.isTempBasalInProgress) {
            aapsLogger.debug(LTag.PUMP, "cancelRealTempBasal: Failed")
            var isok = danaRSService?.tempBasalStop()
            if(isok!!){
