...
open class DanaRSPacket(protected var injector: HasAndroidInjector) {
    ...
    var typeZhiKai = BleEncryption.ZHIKAI_PACKET__TYPE_RESPONSE // most of the messages, should be changed for others
    ...
    val commandZhiKai: Int
        get() = (typeZhiKai and 0xFF shl 8) + (opCode and 0xFF)
    ...
    fun getCommandZhiKai(data: ByteArray): Int {
        val type = byteArrayToInt(getBytes(data, TYPE_START, 1))
        val opCode = byteArrayToInt(getBytes(data, ZHIKAI_OPCODE_START, 1))
        return (type and 0xFF shl 8) + (opCode and 0xFF)
    }
    ...
    companion object {
        ...
        private const val ZHIKAI_OPCODE_START = 4
